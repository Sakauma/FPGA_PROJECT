##################history########################
#version:1.0
#date:2022/05/10
#revision:initial version.
################################################
set XLNX_PROJ_DIR [file join [pwd] "../.."]
set ::env(JFM_PATH) $XLNX_PROJ_DIR
set run_tcl_path [file join $::env(JFM_PATH) "ip_patch" "run.tcl"]
source $run_tcl_path
puts "Start pre_opt_design_patch :"
proc pre_opt_design_patch {} {
    set JDY_top_u0_cell [get_cells -hierarchical -regexp  -filter {REF_NAME=~JDY_ip_top_.* || ORIG_REF_NAME=~JDY_ip_top_.*}]

    if { [get_cells $JDY_top_u0_cell] != ""} {
	    
	    puts "Find $JDY_top_u0_cell cell!"
		
		set XLNX_PROJ_DIR [file join [pwd] "../.."]
		set current_prj_path $XLNX_PROJ_DIR
	    set dcp_path [format "%s/ip_patch/045ai/JDY_ip_top_0_route.dcp" $current_prj_path]
		if {[file exists $dcp_path] == 0} {
		    error "An error occurs:\n Can't find $dcp_path!"
		
		}

		resize_pblock [get_pblocks *jdy_top_agrp] -add { \
		SLICE_X80Y50:SLICE_X139Y149 \
		RAMB36_X4Y10:RAMB36_X5Y29 RAMB18_X4Y20:RAMB18_X5Y59 \
		DSP48_X4Y20:DSP48_X4Y59 \
		} -remove {SLICE_X80Y35:SLICE_X161Y174}



		resize_pblock [get_pblocks *cnu_fifo_agrp] -add { \
		SLICE_X90Y50:SLICE_X139Y74 \
		SLICE_X90Y75:SLICE_X139Y129 \
		} -remove {SLICE_X90Y75:SLICE_X161Y124}	
		set_property CONTAIN_ROUTING true [get_pblocks  *jdy_top_agrp]	
		
		# Prohibit logic placement in AI area to avoid congestion 
		set_property EXCLUDE_PLACEMENT 1 [get_pblocks *cnu_fifo_agrp] -quiet
		create_pblock pblock_forbid
		resize_pblock pblock_forbid -add SLICE_X80Y75:SLICE_X89Y124
		set_property EXCLUDE_PLACEMENT 1 [get_pblocks pblock_forbid]
		
        set_property HD.PARTITION 1 [get_cells $JDY_top_u0_cell]
        update_design -cell [get_cells $JDY_top_u0_cell] -black_box
        read_checkpoint -cell [get_cells $JDY_top_u0_cell] $dcp_path
        lock_design -level routing [get_cells $JDY_top_u0_cell]
		# Disable bitstream CRC 
		set_property BITSTREAM.GENERAL.CRC DISABLE [current_design]
		#Disable DRC check during generating bitstream
		set_property IS_ENABLED 0 [get_drc_checks  REQP-44]
		set_property IS_ENABLED 0 [get_drc_checks  REQP-46]
		# remove PROHIBIT constraints to avoid [Shape Builder 18-119] error during ILA generating
		set_property PROHIBIT false [get_sites -range {SLICE_X32Y75 SLICE_X79Y129}]
		set_property PROHIBIT false [get_sites -range {SLICE_X30Y20 SLICE_X81Y74}]
		set_property PROHIBIT false [get_sites -range {DSP48_X2Y8 DSP48_X3Y52}]
		set_property PROHIBIT false [get_sites -range {RAMB18_X2Y8 RAMB18_X3Y52}]
		set_property PROHIBIT false [get_sites -range {RAMB36_X2Y4 RAMB36_X3Y26}]
        
		set_property PROHIBIT false [get_sites -range {SLICE_X94Y0 SLICE_X97Y29}]
		set_property PROHIBIT false [get_sites -range {SLICE_X92Y24 SLICE_X93Y29}]
		set_property PROHIBIT false [get_sites -range {DSP48_X4Y0 DSP48_X4Y11}]
		set_property PROHIBIT false [get_sites -range {RAMB18_X4Y0 RAMB18_X4Y11}]
		set_property PROHIBIT false [get_sites -range {RAMB36_X4Y0 RAMB36_X4Y5}]
		set_property PROHIBIT false [get_sites -range {DSP48_X6Y0 DSP48_X6Y139}]
		
		set_property PROHIBIT false [get_sites -range {SLICE_X0Y60 SLICE_X32Y90}]
	
	} else {
	    puts "Current project don't need to replace JDY_top_u0 cell!"
	}

}
pre_opt_design_patch
puts "Finish pre_opt_design_patch successfully!"
