...
//--PAD Declarations---------------------------
	...
	output	wire	[31:0]						lvds_cache_cur_waddr						,
	input	wire	[31:0]						lvds_cache_cur_raddr						,
	output										m_axiw_req									,
...
//--Signals define
	...
	reg				[2:0]						video_frame_cnt			= 'b0				;	
//==================================================================================================
//--pre assign
	wire			[31:0]	srio_addr			= {8'h00,lvds_trn_fifo_dout_i [31:16],8'h00} ;

	assign	data_len64							= lvds_trn_fifo_dout_i[0]==1'b1
												? lvds_trn_fifo_dout_i[11:1] + 1'b1
												: lvds_trn_fifo_dout_i[11:1]				;
...
//--LVDS Data write to DDR3
	...
	always @(*) begin
		...
		case(S_AXIW_CM)
			S_AXIW_IDLE_M						: begin
				if(	(!lvds_trn_fifo_empty_i)	//&	
//						(
//								~(
//										((cur_addr + P_LVDS_DDR3_BLOCK_SIZE_R)==lvds_cache_cur_raddr)
//										|
//										(
//											((cur_addr+ P_LVDS_DDR3_BLOCK_SIZE_R)>=P_LVDS_DDR3_END_ADDR_R)	
//											&	
//											(lvds_cache_cur_raddr==P_LVDS_DDR3_START_ADDR_R)
//										) 
//								)	
//						)
				) begin
...
//==================================================================================================
//--trn fifo read	
	assign	lvds_trn_fifo_ren_o					= S_AXIW_CM[B_AXIW_DONE_M]					;
//==================================================================================================
//--Address cache
	always @(posedge sys_clk_i or posedge sys_rst_i) begin
		if(sys_rst_i) begin
			cur_addr							<= P_LVDS_DDR3_START_ADDR_R					;
			video_frame_cnt						<= 'b0										;
		end else if(cur_addr>=P_LVDS_DDR3_END_ADDR_R) begin
			cur_addr							<= P_LVDS_DDR3_START_ADDR_R					;
			video_frame_cnt						<= 'b0										;			
		end else if(S_AXIW_NM[B_AXIW_DONE_M]) begin
		//	cur_addr							<= cur_addr + P_LVDS_DDR3_BLOCK_SIZE_R		;
			
			cur_addr							<= srio_addr	 + P_LVDS_DDR3_BLOCK_SIZE_R	+ video_frame_cnt * P_V_FRAME_DDR3_BLOCK_SIZE_R;

			video_frame_cnt						<= srio_addr== (P_V_FRAME_DDR3_BLOCK_SIZE_R - P_LVDS_DDR3_BLOCK_SIZE_R ) ?  video_frame_cnt + 1	: 	video_frame_cnt			;			
		end else begin
			cur_addr							<= cur_addr									;
			video_frame_cnt						<= video_frame_cnt							;			

		end
	end
	
	assign	lvds_cache_cur_waddr				= cur_addr									;
